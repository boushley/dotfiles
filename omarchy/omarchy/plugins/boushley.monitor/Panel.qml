...
Panel {
  ...
  //   "scale"      - Configurable Button scale presets; treated as a single
  ...
  readonly property var defaultScalePresets: ["1", "1.25", "1.3", "1.4", "1.5", "1.6", "2"]
  property var scalePresets: defaultScalePresets
  ...
  function loadScalePresets(raw) {
    try {
      var parsed = JSON.parse(String(raw || ""))
      var configured = parsed.scales
      ...
      for (var i = 0; i < configured.length; i++) {
        var value = Number(configured[i])
        if (!isFinite(value) || value < 1 || value > 4)
          throw new Error("each scale must be a number from 1 through 4")
      ...
      scales.sort(function(a, b) { return Number(a) - Number(b) })
      root.scalePresets = scales
    } catch (error) {
      console.warn("display-scales.json is invalid, using defaults:", error)
      root.scalePresets = root.defaultScalePresets
    }
  }

  property FileView scalePresetFile: FileView {
    path: Quickshell.env("HOME") + "/.config/omarchy/display-scales.json"
    watchChanges: true
    printErrors: false
    onLoaded: root.loadScalePresets(text())
    onLoadFailed: root.scalePresets = root.defaultScalePresets
