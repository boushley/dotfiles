...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** chooses the strongest evidence for each requirement and drives **`/tdd`** for retained behavior tests (one red-green slice at a time). It then makes a candidate commit, runs **`/code-review`** against the captured starting point, addresses the two-axis findings (Standards + Spec), verifies, and amends the commit. Reach for **`/tdd`** on its own when you just want to build a concrete behavior test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
...
**`/setup-project-for-agents`**: run before your first engineering flow to configure the issue tracker, triage labels, and doc layout the other skills assume. Custom issue trackers also work.
